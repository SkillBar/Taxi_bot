...
|---|----------|------------------|----------------|
| **H1** | **CORS:** preflight OPTIONS не получает нужные заголовки или браузер блокирует ответ GET | Mini App открыт с origin A (например https://твой-webapp.vercel.app), запрос идёт на origin B (taxi-botapi.vercel.app). Без Access-Control-Allow-Origin и т.д. браузер не отдаёт ответ JS. | 1) В логах Vercel (API) смотреть: есть ли запросы OPTIONS и GET при нажатии «Повторить». 2) Вызвать GET https://taxi-botapi.vercel.app/api/ping из того же контекста (Mini App) и посмотреть ответ или ошибку в консоли. |
| **H2** | **Origin в WebView:** Telegram передаёт нестандартный Origin (null, или другой домен), и CORS его не пропускает | В iframe/WebView origin может отличаться от «обычного» браузера. | В ответе /api/ping смотреть, какой origin пришёл. В API явно разрешить известные значения (web.telegram.org и домен WebApp). |
...
| **H5** | **Маршрутизация Vercel:** rewrite (.*) → /api/index не передаёт путь, Fastify получает не тот URL | Тогда наш handler не видит /api/agents/me. | В логах или в ответе /api/ping вывести req.url; убедиться, что до Fastify доходит именно /api/agents/me. |
...
- На **том же телефоне**, где открыт Telegram, в обычном браузере (Chrome/Safari) открыть:
  ...
  - https://taxi-botapi.vercel.app/api/ping  
...
- Вызвать **GET /api/ping** (без авторизации). В ответе должны быть `origin` и, при необходимости, `url`.
- Если из Mini App запрос до /api/ping **доходит** — CORS для простого GET работает; тогда проблема может быть в заголовках для /api/agents/me (например, x-telegram-init-data в preflight).  
- Если до /api/ping **не доходит** (та же «нет связи») — блокировка на этапе CORS preflight или сети.
...
- **GET /api/ping** — диагностический эндпоинт: 200 + JSON `{ ok, origin, url }`. Без авторизации. Нужен, чтобы проверить доходимость и какой Origin приходит.
...
- **Сервер:** /health и (после деплоя) /api/ping отвечают при прямом обращении.
...
- **Действия:** логи Vercel при «Повторить»; проверка /health и /api/ping с телефона; при необходимости явно прописать WEBAPP_ORIGIN и передеплоить API.
