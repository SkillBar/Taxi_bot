...
### Если видите «Нет связи с сервером»

В тексте ошибки теперь показывается **адрес API при сборке** (или «не задан»). Что сделать:

1. **Vercel → проект WebApp** (тот, где собирается фронт) → **Settings → Environment Variables**.
2. Добавьте переменную **`VITE_API_URL`** = URL вашего API **без слэша в конце**, например:  
   `https://taxi-botapi.vercel.app`  
   Область: **Production** (и при необходимости Preview).
3. **Redeploy** WebApp: Deployments → у последнего деплоя → ⋮ → **Redeploy**.  
   Без пересборки новый `VITE_API_URL` не попадёт в приложение (Vite подставляет его на этапе сборки).
4. Откройте Mini App снова из Telegram. Если адрес был «не задан» или неверный — после пунктов 1–3 ошибка должна пропасть.
